...
if(isset($_POST['category']) && isset($_POST['categoryId']) ){
    $query = "INSERT INTO subcategorymaster( subcategoryName, categoryId) VALUES
    ('$category','$categoryId')";
...
if($jobQuery==1)
    {
        ...
        $s = strval($last_id);
        $sql = "SELECT scm.subcategoryId,scm.subcategoryName,scm.categoryId,cm.category	from subcategorymaster scm LEFT JOIN category_master cm ON scm.categoryId =cm.categoryId WHERE scm.subcategoryId = $s";
